...
class GPT(nn.Module):
	...
	def _forward_logits_common(self, input_ids):
	...
	def forward_logits(self,input_ids):
		x = self._forward_logits_common(input_ids)
	...
	def forward(self,input_ids,target_ids):
		if self.training:
			hidden_states = self._forward_logits_common(input_ids)
			return hidden_states.reshape(-1, hidden_states.size(-1))
